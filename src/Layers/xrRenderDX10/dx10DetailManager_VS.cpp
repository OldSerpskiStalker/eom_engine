    ...
    for (u32 O = 0; O < objects.size(); O++)
    {
        ...
        if (!vis.empty())
        {
            ...
            if (!ps_r2_ls_flags.test(R2FLAG_SUN_DETAILS) ||
                ((RImplementation.PHASE_SMAP == RImplementation.phase) ||
                    (RImplementation.PHASE_NORMAL == RImplementation.phase) && (!RImplementation.is_sun())))
                //			if (!ps_r2_ls_flags.test(R2FLAG_SUN_DETAILS) || (!RImplementation.is_sun()))
                vis.clear_not_free();
        }
