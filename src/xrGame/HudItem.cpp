...
void CHudItem::Load(LPCSTR section)
{
    ...
    if (script_ui_funct)
    {
        ...
        script_ui_offset[0] =
            READ_IF_EXISTS(pSettings, r_fvector3, section, "custom_ui_pos", Fvector().set(0.f, 0.f, 0.f));
        script_ui_offset[1] =
            READ_IF_EXISTS(pSettings, r_fvector3, section, "custom_ui_rot", Fvector().set(0.f, 0.f, 0.f));
...
u32 CHudItem::PlayHUDMotion(
    const shared_str& M, BOOL bMixIn, CHudItem* W, u32 state, float speed, float end, bool bMixIn2)
{
...
u32 CHudItem::PlayHUDMotion_noCB(const shared_str& motion_name, BOOL bMixIn, float speed, bool bMixIn2)
{
    ...
    if (HudItemData())
    {
        return HudItemData()->anim_play(
            motion_name, bMixIn, m_current_motion_def, m_started_rnd_anim_idx, speed, bMixIn2);
    }
    else
    {
...
bool CHudItem::TryPlayAnimIdle()
{
    if (MovingAnimAllowedNow())
    {
        ...
        {
            //			if (!smart_cast<CCustomDevice*>(this)) return false;
...
{
    PlayHUDMotion(
        "anm_idle_moving", TRUE, NULL, GetState(), isActorAccelerated(Actor()->MovingState(), false) ? 1.f : .75f);
}
...
void CHudItem::OnMovementChanged(ACTOR_DEFS::EMoveCommand cmd)
{
    ...
    {
        if ((cmd == ACTOR_DEFS::mcSprint) || (cmd == ACTOR_DEFS::mcAnyMove) || (cmd == ACTOR_DEFS::mcCrouch) ||
...
collide::rq_result& CHudItem::GetRQ() { return HUD().GetCurrentRayQuery(); }
