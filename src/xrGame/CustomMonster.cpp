...

    {
        ...
        for (; i != e; ++i)
            Level().debug_renderer().draw_aabb(
                ai().level_graph().vertex_position(*i), radius, radius, radius, D3DCOLOR_XRGB(255, 22, 255));
    }
    ...
    for (; I != E; ++I)
    {
        ...
        for (u32 i = 0; i < sizeof(z_offs) / sizeof(z_offs[0]); ++i)
            Level().debug_renderer().draw_line(
                Fidentity, traj_start + z_offs[i], traj_end + z_offs[i], D3DCOLOR_XRGB(255, 255, 128));
        ...
        for (u32 i = 0; i < sizeof(x_offs) / sizeof(x_offs[0]); ++i)
            Level().debug_renderer().draw_line(
                Fidentity, hor_start + x_offs[i], hor_end + x_offs[i], D3DCOLOR_XRGB(255, 255, 128));
        ...
        for (u32 i = 0; i < sizeof(y_offs) / sizeof(y_offs[0]); ++i)
            Level().debug_renderer().draw_line(
                Fidentity, ver_start + y_offs[i], ver_end + y_offs[i], D3DCOLOR_XRGB(255, 255, 128));
