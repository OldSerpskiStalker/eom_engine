...
void CScriptEntity::SetScriptControl(const bool bScriptControl, shared_str caSciptName)
{
    ...
    #ifdef DEBUG
        if (bScriptControl)
            ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeInfo,"Script %s set object %s under
    its control",*caSciptName,*object().cName()); else ai().script_engine().script_log
    (ScriptStorage::eLuaMessageTypeInfo,"Script %s freed object %s from its control",*caSciptName,*object().cName());
    #endif
    */
    if (!bScriptControl)
