    ...
    u32& current_travel_point, float& dist, float& dist_to_target, Fvector& dir_to_target)
{
    ...
    //			 èçìåíèòü detail().m_current_travel_point

    float desirable_speed = velocity; // æåëàåìàÿ ñêîðîñòü îáúåêòà
    dist = desirable_speed * time_delta; // ïðîéäåííîå ðàññòîÿíèå â ñîîñòâåòñòâèå ñ æåëàåìîé ñêîðîñòüþ

    // îïðåäåëèòü öåëåâóþ òî÷êó
    Fvector target;

    // îáíîâèòü detail().m_current_travel_point â ñîîòâåòñòâèå ñ òåêóùåé ïîçèöèåé
...
#pragma todo("Dima to ? : is this correct?")
    ...
    // Ôèçèêà óñòàíàâëèâàåò íîâóþ ïîçèöèþ
    Device.Statistic->Physics.Begin();

    // ïîëó÷èòü ôèç. îáúåêòû â ðàäèóñå
    ...
    if (velocity.y < -0.9f)
        velocity.y = -0.8f;
    velocity.normalize_safe(); // êàê íå ñòðàííî, mdir - íå íîðìèðîâàí
    ...
    // Ôèçèêà óñòàíàâëèâàåò ïîçèöèþ â ñîîòâåòñòâèè ñ íóëåâîé ñêîðîñòüþ
