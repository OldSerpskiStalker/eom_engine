...
void CPHMovementControl::TraceBorder(const Fvector& prev_position)
{
    ...
    g_pGameLevel->ObjectSpace.RayQuery(
        storage, RD, BorderTraceCallback, &p, NULL, smart_cast<CObject*>(m_character->PhysicsRefObject()));
}
...
//&c);// 	CPHObject* obj = (CPHObject*)my_data->ph_object; 	VERIFY( obj ); 	VERIFY( obj->Island().DActiveIsland() !=
//&(obj->Island()) ); 	VERIFY( !obj->Island().IsActive() ); 	obj->Island().DActiveIsland()->ConnectJoint(contact_joint);
//
//	obj->EnableObject(0);
//	if(bo1)
