...
void CUITextureMaster::ParseShTexInfo(LPCSTR xml_file)
{
    ...
    for (int fi = 0; fi < files_num; ++fi)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            /* avo: fix issue when values were not updated (silently skipped) when same key is encountered more than
            once. This is how std::map is designed.
            /* Also used more efficient C++11 std::map::emplace method instead of outdated std::pair::make_pair */
            /* XXX: avo: note that xxx.insert(mk_pair(v1,v2)) pattern is used extensively throughout solution so there
             * is a good potential for other bug fixes/improvements */
