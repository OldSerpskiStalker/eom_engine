...
CUIActorMenu* GetActorMenu() { return &CurrentGameUI()->GetActorMenu(); }

CUIPdaWnd* GetPDAMenu() { return &CurrentGameUI()->GetPdaMenu(); }

u8 GrabMenuMode() { return (u8)(CurrentGameUI()->GetActorMenu().GetMenuMode()); }
...
void CUIActorMenu::HighlightSectionInSlot(LPCSTR section, u8 type, u16 slot_id)
{
    ...
    switch (type)
    {
    case EDDListType::iActorBag: slot_list = m_pInventoryBagList; break;
    case EDDListType::iActorBelt: slot_list = m_pInventoryBeltList; break;
    case EDDListType::iActorSlot: slot_list = GetSlotList(slot_id); break;
    case EDDListType::iActorTrade: slot_list = m_pTradeActorBagList; break;
    case EDDListType::iDeadBodyBag: slot_list = m_pDeadBodyBagList; break;
    case EDDListType::iPartnerTrade: slot_list = m_pTradePartnerList; break;
    case EDDListType::iPartnerTradeBag: slot_list = m_pTradePartnerBagList; break;
    case EDDListType::iQuickSlot: slot_list = m_pQuickSlot; break;
    case EDDListType::iTrashSlot: slot_list = m_pTrashList; break;
    }
...
{
    module(
        L)[class_<enum_exporter<EDDListType>>("EDDListType")
               .enum_("EDDListType")[value("iActorBag", int(EDDListType::iActorBag)),
                   value("iActorBelt", int(EDDListType::iActorBelt)), value("iActorSlot", int(EDDListType::iActorSlot)),
                   value("iActorTrade", int(EDDListType::iActorTrade)),
                   value("iDeadBodyBag", int(EDDListType::iDeadBodyBag)), value("iInvalid", int(EDDListType::iInvalid)),
    ...
    module(L, "ActorMenu")[def("get_pda_menu", &GetPDAMenu), def("get_actor_menu", &GetActorMenu),
