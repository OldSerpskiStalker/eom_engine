...
bool CUIMapWnd::OnMouseAction(float x, float y, EUIMessages mouse_action)
{
    ...
    if (GlobalMap() && !GlobalMap()->Locked() && ActiveMapRect().in(cursor_pos1))
    {
        switch (mouse_action)
        {
        case WINDOW_RBUTTON_UP: ActivatePropertiesBox(NULL); break;
...
void CUIMapWnd::SendMessage(CUIWindow* pWnd, s16 msg, void* pData)
{
    ...
    if (pWnd == m_UIPropertiesBox && msg == PROPERTY_CLICKED && m_UIPropertiesBox->GetClickedItem())
    {
        ...
        {
            ShowSettingsWindow(
                m_cur_location->ObjectID(), m_cur_location->GetLastPosition(), m_cur_location->GetLevelName());
            m_cur_location = NULL;
            break;
        }
...
void CUIMapWnd::ActivatePropertiesBox(CUIWindow* w)
{
    ...
    if (m_cur_location->IsUserDefined())
    {
        m_UIPropertiesBox->AddItem(
            "st_pda_change_spot_hint", NULL, MAP_CHANGE_SPOT_HINT_ACT); // Изменяем название метки
...
// Alun: Correct now. All you need is relative mouse position to absolute pos of uilevelmap, then remove widescreen
// scale on X before local-to-world convert
bool CUIMapWnd::ConvertCursorPosToMap(Fvector* return_position, CUILevelMap* curr_map)
{
    ...
    return_position->z =
        bound_rect.height() + bound_rect.lt.y - cursor_pos.y / (box_rect.height() / bound_rect.height());
