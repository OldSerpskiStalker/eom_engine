...
CUIInventoryCellItem::CUIInventoryCellItem(CInventoryItem* itm)
{
    ...
    while (pSettings->line_exist(itm->m_section_id, field))
    {
        ...
        offset.x = pSettings->r_float(
            itm->m_section_id, strconcat(sizeof(buf), buf, std::to_string(itrNum).c_str(), "icon_layer_x"));
        offset.y = pSettings->r_float(
            itm->m_section_id, strconcat(sizeof(buf), buf, std::to_string(itrNum).c_str(), "icon_layer_y"));
        ...
        // u32 color = pSettings->line_exist(itm->m_section_id, field_color) ? pSettings->r_color(itm->m_section_id,
        // field_color) : 0;
...
void CUIInventoryCellItem::OnAfterChild(CUIDragDropListEx* parent_list)
{
    ...
    {
        (*it)->m_icon =
            InitLayer((*it)->m_icon, (*it)->m_name, (*it)->offset, parent_list->GetVerticalPlacement(), (*it)->m_scale);
    }
}
...
CUIStatic* CUIInventoryCellItem::InitLayer(
    CUIStatic* s, LPCSTR section, Fvector2 addon_offset, bool b_rotate, float scale)
{
    if (!s)
    {
        s = xr_new<CUIStatic>();
