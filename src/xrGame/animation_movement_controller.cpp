...
void animation_movement_controller::InitalPositionBlending(const Fmatrix& to)
{
    ...
    /*
       ...
       blend_angular_speed*Device.fTimeDelta, 0.00001, 0.000001 ); m_pObjXForm.set( res );
    */
...
)->motionID ).first; LPCSTR old_anim_set		= m_pKinematicsC->dcast_PKinematicsAnimated( )->LL_MotionDefName_dbg(
ControlBlend( )->motionID ).second; LPCSTR new_anim_name	= m_pKinematicsC->dcast_PKinematicsAnimated(
)->LL_MotionDefName_dbg( B->motionID ).first; LPCSTR new_anim_set		= m_pKinematicsC->dcast_PKinematicsAnimated(
)->LL_MotionDefName_dbg( B->motionID ).second;

    if( ControlBlend( )->playing )
        Msg( " ! obj movement anim not yet ended anim: %s anim set: %s \n and already another started anim: %s anim set:
%s", new_anim_name,new_anim_set,old_anim_name,old_anim_set
            );
...
anim: %s anim set: %s", old_anim_name,old_anim_set,new_anim_name,new_anim_set
            );
    if( !B->stop_at_end )
        Msg( " ! obj movement anim  : %s anim set: %s  is not stop-at-end but fallowing after another obj movement anim:
%s anim set: %s", new_anim_name,new_anim_set,old_anim_name,old_anim_set
            );
#endif
    //	VERIFY(  );
