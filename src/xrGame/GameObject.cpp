    ...
    else
    {
        ...
        if (o != NULL)
        {
            Msg("ERROR: CGameObject:net_spawn() Object with ID already exists! ID=%d self=%s other=%s", E->ID,
                *(cName()), *(o->cName()));
            // ai().script_engine().script_log(eLuaMessageTypeError, "CGameObject:net_Spawn() |
            // Level().Objects.net_Find(E->ID) != NULL (This mean object already exist on level by this ID) ID=%d
