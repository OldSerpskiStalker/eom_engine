...
IC void stalker_movement_manager_base::set_mental_state(EMentalState mental_state)
{
    ...
    // #pragma todo("Dima to Dima: this is correct, commented just because of the October presentation, no time right
    // now to fix it correctly, should be fixed sometimes later") .	m_path_actuality			= m_path_actuality &&
    //(m_target.m_mental_state == m_current.m_mental_state);
}
