...
void CScriptGameObject::explode(u32 level_time)
{
    ...
    if (object().H_Parent())
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CExplosive : cannot explode object wiht parent!");
        return;
    }

    if (!explosive)
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CExplosive : cannot access class member explode!");
    else
    {
        Fvector normal;
...
bool CScriptGameObject::active_zone_contact(u16 id)
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CScriptZone : cannot access class member active_zone_contact!");
        return (false);
    }
...
CScriptGameObject* CScriptGameObject::best_weapon()
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CScriptEntity : cannot access class member best_weapon!");
        return (0);
    }
    else
    {
...
void CScriptGameObject::play_cycle(LPCSTR anim, bool mix_in)
{
    ...
    if (sa)
    {
        ...
        {
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError, "CGameObject : has not cycle %s", anim);
        }
    }
    else
    {
...
u32 CScriptGameObject::memory_time(const CScriptGameObject& lua_game_object)
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CScriptEntity : cannot access class member memory!");
        return (0);
    }
    else
...
const xr_vector<CNotYetVisibleObject>& CScriptGameObject::not_yet_visible_objects() const
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CGameObject : cannot access class member not_yet_visible_objects!");
        NODEFAULT;
    }
...
float CScriptGameObject::visibility_threshold() const
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CGameObject : cannot access class member visibility_threshold!");
        NODEFAULT;
    }
...
void CScriptGameObject::enable_vision(bool value)
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CVisualMemoryManager : cannot access class member enable_vision!");
        return;
    }
...
bool CScriptGameObject::vision_enabled() const
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CVisualMemoryManager : cannot access class member vision_enabled!");
        return (false);
    }
...
float CScriptGameObject::ignore_monster_threshold() const
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : cannot access class member ignore_monster_threshold!");
        return (0.f);
    }
...
CCar* CScriptGameObject::get_car()
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CGameObject : cannot access class member get_car!");
        NODEFAULT;
    }
    return car;
}
...
void CScriptGameObject::debug_planner(const script_planner* planner)
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : cannot access class member debug_planner!");
        return;
    }
...
u32 CScriptGameObject::location_on_path(float distance, Fvector* location)
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : location_on_path -> specify destination location!");
        return (u32(-1));
    }
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : cannot access class member location_on_path!");
        return (u32(-1));
    }
...
bool CScriptGameObject::is_there_items_to_pickup() const
{
    ...
    {
        ai().script_engine().script_log(
            ScriptStorage::eLuaMessageTypeError, "CAI_Stalker : cannot access class member is_there_items_to_pickup!");
        return false;
    }
