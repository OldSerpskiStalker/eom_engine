...
// void CEntity::Hit			(float perc, Fvector &dir, CObject* who, s16 element,Fvector position_in_object_space, float
...
void CEntity::KillEntity(u16 whoID, BOOL bypass_actor_check /*AVO: added for actor_before_death callback*/)
{
    ...
    // IMPORTANT: if you wish to kill actor you need to call db.actor:kill(level:object_by_id(whoID), true) in
    // actor_before_death callback, to ensure all objects are properly destroyed
