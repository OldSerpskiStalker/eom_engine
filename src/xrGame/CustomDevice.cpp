...
void CCustomDevice::ForceHide() { g_player_hud->detach_item(this); }
...
void CCustomDevice::OnStateSwitch(u32 S, u32 oldState)
{
    ...
    case eHiding: {
        if (oldState != eHiding)
        {
            ...
            m_fZoomfactor > .5f && (oldState == eIdleZoom || oldState == eIdleZoomIn || oldState == eIdleZoomOut) &&
                    HudAnimationExist(m_bFastAnimMode ? "anm_zoom_hide_fast" : "anm_zoom_hide") ?
    ...
    case eIdleZoomIn: {
        m_bZoomed = true;
        HudAnimationExist("anm_zoom") ? PlayHUDMotion("anm_zoom", TRUE, this, GetState()) : SwitchState(eIdleZoom);
        SetPending(FALSE);
    }
    break;
    case eIdleZoomOut: {
        m_bZoomed = false;
        HudAnimationExist("anm_zoom") ? PlayHUDMotion("anm_zoom", TRUE, this, GetState()) : SwitchState(eIdle);
        SetPending(FALSE);
    }
    break;
...
void CCustomDevice::OnActiveItem() { return; }
...
bool CCustomDevice::IsWorking() { return m_bWorking && H_Parent() && H_Parent() == Level().CurrentViewEntity(); }
            ...
            else
            {
                ...
                if (wpn)
                {
                    ...
                    if (GetState() == eIdleZoomIn || GetState() == eIdleZoomOut)
                    {
                        ...
                        {
                            PlayHUDMotion(
                                "anm_idle", FALSE, this, eIdle); // Necessary to fix small jump caused by motion mixing
                            SwitchState(eIdle);
                        }
                    }
                }
...
void CCustomDevice::TurnDeviceInternal(bool b) { m_bWorking = b; }
...
void CCustomDevice::UpdateHudAdditional(Fmatrix& trans)
{
    ...
    float fStrafeMinAngle =
        _lerp(hi->m_measures.m_strafe_offset[3][0].y, hi->m_measures.m_strafe_offset[3][1].y, m_fZoomfactor);

    // Высчитываем мксимальный наклон от поворота камеры
    float fCamLimitBlend =
        _lerp(hi->m_measures.m_strafe_offset[3][0].x, hi->m_measures.m_strafe_offset[3][1].x, m_fZoomfactor);
...
bool CCustomDevice::render_item_3d_ui_query() { return IsWorking(); }
