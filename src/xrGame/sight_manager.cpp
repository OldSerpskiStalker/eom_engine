...
Fvector CSightManager::aiming_position() const
{
    ...
    case eSightTypeDirection: {
        ...
        VERIFY2(result.magnitude() < 100000.f,
            make_string("[%f][%f][%f] [%f][%f][%f] [%f]", VPUSH(object().Position()),
...
void CSightManager::process_action(float const time_delta)
{
    ...
    //factors.y, s_factor_lerp_speed*time_delta ), factors.y ); 		Msg							("[%6d][%6d] [%f] + [%f] =
    //[%f] ([%f])",  Device.dwFrame, Device.dwTimeGlobal, m_current.m_spine.m_factor,	s_factor_lerp_speed*time_delta,
    //lerp ( m_current.m_spine.m_factor,		factors.z, s_factor_lerp_speed*time_delta ), factors.z );
    //	}
