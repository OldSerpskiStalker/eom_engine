...
Upgrade* Manager::add_upgrade(shared_str const& upgrade_id, Group& parent_group)
{
    ...
    {
        VERIFY2(0,
            make_string("Try add the existent upgrade (%s), in group <%s>. Such upgrade is in group <%s> already!",
...
LPCSTR Manager::get_upgrade_by_index(CInventoryItem& item, Ivector2 const& index)
{
    ...
    VERIFY2(upgrade,
        make_string("! Upgrade with index <%d,%d> in inventory item [%s] does not exist!", index.x, index.y,
