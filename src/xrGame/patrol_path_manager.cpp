...
void CPatrolPathManager::select_point(const Fvector& position, u32& dest_vertex_id)
{
...
#if 0
		// åñëè âûáðàííàÿ íîäà íå ñîîòâåòñòâóåò òåêóùåé íîäå - âñå îê
		// èíà÷å âûáðàòü ñëåäóþùóþ âåðøèíó ïàòðóëüíîãî ïóòè
    ...
    u32 count = 0; // êîëè÷åñòâî ðàçâåòâëåíèé
    float sum = 0.f; // ñóììà âåñîâ ðàçâåòâëåíèÿ
    vertex = m_path->vertex(m_curr_point_index);
    CPatrolPath::const_iterator I = vertex->edges().begin(), E = vertex->edges().end();
    u32 target = u32(-1);

    // âû÷èñëèòü êîëè÷åñòâî ðàçâåòâëåíèé
    for (; I != E; ++I)
    {
...
shared_str CPatrolPathManager::path_name() const
{
    if (!m_path)
    {
        ai().script_engine().script_log(
            eLuaMessageTypeError, "Path not specified (object %s)!", *m_game_object->cName());
        return ("");
    }
    VERIFY(m_path);
    return (m_path_name);
