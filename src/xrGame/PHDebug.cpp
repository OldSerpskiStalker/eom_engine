...
struct SPHObjDBGDraw : public SPHDBGDrawAbsract
{
    ...
    void render() { Level().debug_renderer().draw_aabb(AABB_center, AABB.x, AABB.y, AABB.z, D3DCOLOR_XRGB(255, 0, 0)); }
    Fvector AABB;
    Fvector AABB_center;
};
...
class CPHDebugOutput : public IDebugOutput
{
    ...
    virtual u32& dbg_tries_num()
    {
        ...
        //form._14_ )	+ 	make_string( "%s, _24_=%f \n", dump_string( make_string( "%s.j, ", name ).c_str(), form.j
        //).c_str( ) , form._24_ )	+ 	make_string( "%s, _34_=%f \n", dump_string( make_string( "%s.k, ", name ).c_str(),
        //form.k ).c_str( ) , form._34_  ) + 	make_string( "%s, _44_=%f \n", dump_string( make_string( "%s.c, ", name
        //).c_str(), form.c ).c_str( ) , form._44_ );
    }
