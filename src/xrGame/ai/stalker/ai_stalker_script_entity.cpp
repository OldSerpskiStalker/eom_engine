    ...
    case eObjectActionReload1: {
        ...
        if (inventory().ActiveItem()->object().ID() == l_tObjectAction.m_tpObject->ID())
        {
            ...
            ai().script_engine().script_log(
                ScriptStorage::eLuaMessageTypeError, "cannot reload active item because it is not selected!");
        ...
        //			}
        //			else
        //				ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"cannot reload active item because
        //it is not selected!");
        break;
    }
