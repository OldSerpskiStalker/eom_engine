    ...
    CAI_Stalker& self, Fmatrix const& transform, IKinematicsAnimated* kinematics_animated, LPCSTR animation_id)
{
    ...
    draw_bones(*kinematics, Fvector().set(.011f, .011f, .011f), D3DCOLOR_XRGB(0, 255, 0), D3DCOLOR_XRGB(0, 255, 255),
        &transform);
#endif // #ifdef DEBUG_RENDER
    ...
    draw_bones(*kinematics, Fvector().set(.01f, .01f, .01f), D3DCOLOR_XRGB(0, 0, 255), D3DCOLOR_XRGB(255, 255, 0));

    renderer.draw_obb(g_stalker_skeleton[weapon_bone_id0], Fvector().set(.01f, .01f, .01f), D3DCOLOR_XRGB(255, 0, 0));
    renderer.draw_obb(g_stalker_skeleton[weapon_bone_id1], Fvector().set(.01f, .01f, .01f), D3DCOLOR_XRGB(255, 0, 0));
    renderer.draw_line(Fidentity, g_stalker_skeleton[weapon_bone_id0].c, g_stalker_skeleton[weapon_bone_id1].c,
        D3DCOLOR_XRGB(255, 0, 0));
#endif // #ifdef DEBUG_RENDER
}
...
//	draw_animation_bones		(*this, XFORM(), kinematics, "loophole_3_attack_in_0");
        ...
        ai().level_graph().valid_vertex_position(memory().danger().selected()->position()))
    {
        ...
        Level().debug_renderer().draw_aabb(position, half_size - .01f, 1.f,
            ai().level_graph().header().cell_size() * .5f - .01f, D3DCOLOR_XRGB(0 * 255, 255, 0 * 255));
