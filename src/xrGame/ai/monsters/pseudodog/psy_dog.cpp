...
bool CPsyDog::spawn_phantom()
{
    ...
    LPCSTR phantomSection =
        READ_IF_EXISTS(pSettings, r_string, this->get_section(), "phantom_section", "psy_dog_phantom");
...
void CPsyDogPhantom::Think()
{
    ...
    Actor()->Cameras().AddCamEffector(xr_new<CMonsterEffectorHit>(m_appear_effector.ce_time,
        m_appear_effector.ce_amplitude, m_appear_effector.ce_period_number, m_appear_effector.ce_power));
...
// void CPsyDogPhantom::Hit(float P,Fvector &dir,CObject*who,s16 element,Fvector p_in_object_space,float impulse,
