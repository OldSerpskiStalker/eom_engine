...
void CControlAnimationBase::reinit()
{
    ...
    // îáíîâèòü êîëè÷åñòâî àíèìàöèé
    m_anim_motion_map.clear();
    UpdateAnimCount();

    // èíèöèàëèçàöèÿ èíôîðìàöèè î òåêóùåé àíèìàöèè
...
void CControlAnimationBase::select_animation(bool anim_end)
{
    ...
    // ïåðåêðûòü âñå îïðåäåëåíèÿ è óñòàíîâòü àíèìàöèþ
    m_object->ForceFinalAnimation();

    // ïîëó÷èòü ýëåìåíò SAnimItem, ñîîòâåòñòâóþùèé òåêóùåé àíèìàöèè
    SAnimItem* anim_it = m_anim_storage[cur_anim_info().get_motion()];
    VERIFY(anim_it);

    // îïðåäåëèòü íåîáõîäèìûé èíäåêñ
    int index;
...
// ïðîâåðèòü ñóùåñòâóåò ëè ïåðåõîä èç àíèìàöèè from â to
bool CControlAnimationBase::CheckTransition(EMotionAnim from, EMotionAnim to)
{
    ...
    // ïîèñê ñîîòâåòñòâóþùåãî ïåðåõîäà
    ...
    while (!bVectEmpty)
    {
        // âõîä â öèêë, åñëè âåêòîð ïåðåõîäîâ íå ïóñòîé
        ...
        if (from_is_good && target_is_good)
        {
            ...
            if (I->chain)
            {
                ...
                I = m_tTransitions.begin(); // íà÷àòü ñíà÷àëà
                continue;
            }
            else
                break;
...
{
    // èñêàòü òåêóùóþ àíèìàöèþ â AA_VECTOR
...
void CControlAnimationBase::UpdateAnimCount()
{
    ...
    for (ANIM_ITEM_VECTOR_IT it = m_anim_storage.begin(); it != m_anim_storage.end(); it++)
    {
        ...
        // ïðîâåðèòü, áûëè ëè óæå çàãðóæåíû äàííûå
        if ((*it)->count != 0)
            return;
