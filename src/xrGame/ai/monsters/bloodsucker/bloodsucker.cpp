...
void CAI_Bloodsucker::vfAssignBones()
{
    ...
    if (!PPhysicsShell()) // íåëüçÿ ñòàâèòü êîëáåêè, åñëè ñîçäàí ôèç øåë - ó íåãî ñòîÿò ñâîè êîëáåêè!!!
    {
...
{
    //// ïîëó÷àåì âåêòîð íàïðàâëåíèÿ ê èñòî÷íèêó çâóêà è åãî ìèðîâûå óãëû
    // float		yaw,pitch;
    // to_dir.getHP(yaw,pitch);

    //// óñòàíîâèòü ïàðàìåòðû âðàùåíèÿ ïî yaw
    // float cur_yaw = -movement().m_body.current.yaw;						// òåêóùèé ìèðîâîé óãîë ìîíñòðà
    // float bone_angle;											// óãîë äëÿ áîíû

    // float dy = _abs(angle_normalize_signed(yaw - cur_yaw));		// äåëüòà, íà êîòîðóþ íóæíî ïîâîðà÷èâàòüñÿ
    ...
    //// óñòàíîâèòü ïàðàìåòðû âðàùåíèÿ ïî pitch
