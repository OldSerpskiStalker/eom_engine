...
void ChimeraAttackState<Object>::execute()
{
    ...
    bool const can_prepare_jump =
        preparing_state && current_time() > m_last_jump_time + object->get_attack_params().prepare_jump_timeout;
    bool const can_attack_jump =
        !preparing_state && current_time() > m_last_jump_time + object->get_attack_params().attack_jump_timeout;
    ...
    if (m_state == state_rotate)
    {
        ...
        bool const in_stealth = current_time() < m_stealth_end_tick && !object->EnemyMan.enemy_see_me_now();
        if (!in_stealth)
            m_stealth_end_tick = 0;
