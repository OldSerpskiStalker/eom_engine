...
void bonesBone::Turn(u32 dt)
{
    ...
    float dy;
    dy = cur_speed * dt / 1000; // ó÷èòûâàåì ìèëèñåê è ðàäèàííóþ ìåðû
...
void bonesBone::Apply()
{
    ...
    // ñîçäàòü ìàòðèöó âðàùåíèÿ è óìíîæèòü íà mTransform áîíû
    Fmatrix M;
    M.setHPB(-y, -x, -z);
...
{
    // ïðîâåñòè îáðàáîòêó âñåõ êîñòåé
    ...
    // åñëè ïðîöåññ âîçâðàòà çàâåðøåí
    ...
    // åñëè íè÷åãî íå ïðîèçîøëî - âûõîä
    if (!bActive && !bones_were_turned)
        return;

    // åñëè âûïîëíÿåòñÿ íàðàùèâàíèå óãëà è íè îäíà êîñòü íå ïîâåðíóëàñü (äîñòèãëè òàðãåòà...)
    if (!bones_were_turned && !in_return_state)
    {
        ...
        if ((0 != time_started) && (time_started + freeze_time < cur_time))
        {
            ...
            // óñòàíîâèòü ó âñåõ êîñòåé â m_Bone òàðãåòû â 0
            for (u32 i = 0; i < m_Bones.size(); ++i)
            {
    ...
    // Óñòàíîâèòü ïàðàìåòðû èç m_Bones
    for (u32 i = 0; i < m_Bones.size(); ++i)
    {
        if (m_Bones[i].bone == bone)
            m_Bones[i].Apply();
