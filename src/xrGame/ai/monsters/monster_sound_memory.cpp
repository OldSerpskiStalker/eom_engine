...
struct pred_remove_nonactual_sounds
{
    ...
    {
        // óäàëèòü çâóêè îò îáúåêòîâ, ïåðåøåäøèõ â îôôëàéí
        if (x.who && x.who->getDestroy())
            return true;

        // óäàëèòü 'ñòàðûå' çâóêè
        if (x.time < new_time)
            return true;

        // óäàëèòü çâóêè îò íåæèâûõ îáúåêòîâ
        if (x.who)
        {
