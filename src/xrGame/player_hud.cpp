...
void attachable_hud_item::setup_firedeps(firedeps& fd)
{
    ...
    if (m_measures.m_prop_flags.test(hud_item_measures::e_fire_point))
    {
        ...
        fire_mat.transform_tiny(fd.vLastFP,
            m_parent->m_adjust_mode ? m_parent->m_adjust_firepoint_shell[0][0] : m_measures.m_fire_point_offset);
        m_item_transform.transform_tiny(fd.vLastFP);

        fd.vLastFD.set(
            m_parent->m_adjust_mode ? m_parent->m_adjust_firepoint_shell[1][0] : m_measures.m_fire_direction);
    ...
    if (m_measures.m_prop_flags.test(hud_item_measures::e_fire_point2))
    {
        ...
        fire_mat.transform_tiny(fd.vLastFP2,
            m_parent->m_adjust_mode ? m_parent->m_adjust_firepoint_shell[0][1] : m_measures.m_fire_point2_offset);
    ...
    if (m_measures.m_prop_flags.test(hud_item_measures::e_shell_point))
    {
        ...
        fire_mat.transform_tiny(fd.vLastSP,
            m_parent->m_adjust_mode ? m_parent->m_adjust_firepoint_shell[1][1] : m_measures.m_shell_point_offset);
...
void hud_item_measures::load(const shared_str& sect_name, IKinematics* K)
{
    ...
    m_inertion_params.m_tendto_ret_speed =
        READ_IF_EXISTS(pSettings, r_float, sect_name, "inertion_tendto_ret_speed", TENDTO_SPEED_RET);
    m_inertion_params.m_tendto_ret_speed_aim =
        READ_IF_EXISTS(pSettings, r_float, sect_name, "inertion_tendto_ret_aim_speed", TENDTO_SPEED_RET_AIM);
    ...
    m_inertion_params.m_min_angle_aim =
        READ_IF_EXISTS(pSettings, r_float, sect_name, "inertion_min_angle_aim", INERT_MIN_ANGLE_AIM);

    m_inertion_params.m_offset_LRUD =
        READ_IF_EXISTS(pSettings, r_fvector4, sect_name, "inertion_offset_LRUD", Fvector4().set(ORIGIN_OFFSET));
    m_inertion_params.m_offset_LRUD_aim =
        READ_IF_EXISTS(pSettings, r_fvector4, sect_name, "inertion_offset_LRUD_aim", Fvector4().set(ORIGIN_OFFSET_AIM));
    ...
    m_shooting_params.m_shot_max_offset_LRUD =
        READ_IF_EXISTS(pSettings, r_fvector4, sect_name, "shooting_max_LRUD", Fvector4().set(0, 0, 0, 0));
    m_shooting_params.m_shot_max_offset_LRUD_aim =
        READ_IF_EXISTS(pSettings, r_fvector4, sect_name, "shooting_max_LRUD_aim", Fvector4().set(0, 0, 0, 0));
    m_shooting_params.m_shot_offset_BACKW =
        READ_IF_EXISTS(pSettings, r_fvector2, sect_name, "shooting_backward_offset", Fvector2().set(0, 0));
...
u32 attachable_hud_item::anim_play(
    const shared_str& anm_name_b, BOOL bMixIn, const CMotionDef*& md, u8& rnd_idx, float speed, bool bMixIn2)
{
...
player_hud::player_hud()
{
    ...
    for (int i = 0; i < move_anms_end; i++)
    {
        ...
        R_ASSERT2(pSettings->line_exist("hud_movement_layers", temp),
            make_string("Missing definition for [hud_movement_layers] %s", temp));
...
{
    bool b_r0 = ((m_attached_items[0] && m_attached_items[0]->need_renderable()) || script_anim_part == 0 ||
        script_anim_part == 2);
    bool b_r1 = ((m_attached_items[1] && m_attached_items[1]->need_renderable()) || script_anim_part == 1 ||
        script_anim_part == 2);
...
void player_hud::update(const Fmatrix& cam_trans)
{
    ...
    float control_factor = 0.f;
    float old_torso_yaw = 0.f;
    ...
    if (control_factor > 0)
    {
        ...
        float body_yaw = -angle_normalize_signed(old_torso_yaw);
    ...
    if ((m_attached_items[0] || m_attached_items[1]) && control_factor > 0 && sub_z < 0)
    {
        ...
        z1 = (g_freelook_z_offset ? g_freelook_z_offset :
                                    m_attached_items[m_attached_items[0] ? 0 : 1]->m_measures.m_fFreelookZOffset);
        z2 = (g_freelook_z_offset ? g_freelook_z_offset :
                                    m_attached_items[m_attached_items[1] ? 1 : 0]->m_measures.m_fFreelookZOffset);
...
u32 player_hud::anim_play(
    u16 part, const MotionID& M, BOOL bMixIn, const CMotionDef*& md, float speed, u16 override_part)
{
    u16 part_id = u16(-1);
...
u32 player_hud::script_anim_play(u8 hand, LPCSTR section, LPCSTR anm_name, bool bMixIn, float speed)
{
    ...
    if (pSettings->line_exist(section, "item_visual"))
    {
        script_anim_item_model =
            ::Render->model_Create(pSettings->r_string(section, "item_visual"))->dcast_PKinematicsAnimated();
