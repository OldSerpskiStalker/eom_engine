...
public:
    ...
    void setPosition(dReal x, dReal y, dReal z) { dGeomSetPosition(_id, x, y, z); }
    ...
    void setQuaternion(const dQuaternion quat) { dGeomSetQuaternion(_id, quat); }
    void getQuaternion(dQuaternion quat) const { dGeomGetQuaternion(_id, quat); }
...
public:
    ...
    void collide(void* data, dNearCallback* callback) { dSpaceCollide(id(), data, callback); }
};
...
public:
    dSimpleSpace(dSpaceID space) { _id = (dGeomID)dSimpleSpaceCreate(space); }
};
...
public:
    ...
    void setLevels(int minlevel, int maxlevel) { dHashSpaceSetLevels(id(), minlevel, maxlevel); }
};
...
public:
    dSphere() {}
    dSphere(dSpaceID space, dReal radius) { _id = dCreateSphere(space, radius); }
...
public:
    dBox() {}
    dBox(dSpaceID space, dReal lx, dReal ly, dReal lz) { _id = dCreateBox(space, lx, ly, lz); }
    ...
    void setLengths(dReal lx, dReal ly, dReal lz) { dGeomBoxSetLengths(_id, lx, ly, lz); }
    void getLengths(dVector3 result) const { dGeomBoxGetLengths(_id, result); }
};
...
public:
    dPlane() {}
    dPlane(dSpaceID space, dReal a, dReal b, dReal c, dReal d) { _id = dCreatePlane(space, a, b, c, d); }
    ...
    void getParams(dVector4 result) const { dGeomPlaneGetParams(_id, result); }
};
...
public:
    dCCylinder() {}
    dCCylinder(dSpaceID space, dReal radius, dReal length) { _id = dCreateCCylinder(space, radius, length); }
    ...
    void setParams(dReal radius, dReal length) { dGeomCCylinderSetParams(_id, radius, length); }
    void getParams(dReal* radius, dReal* length) const { dGeomCCylinderGetParams(_id, radius, length); }
};
...
public:
    ...
    void set(dReal px, dReal py, dReal pz, dReal dx, dReal dy, dReal dz) { dGeomRaySet(_id, px, py, pz, dx, dy, dz); }
    void get(dVector3 start, dVector3 dir) { dGeomRayGet(_id, start, dir); }

    void setParams(int firstContact, int backfaceCull) { dGeomRaySetParams(_id, firstContact, backfaceCull); }
    void getParams(int* firstContact, int* backfaceCull) { dGeomRayGetParams(_id, firstContact, backfaceCull); }
    void setClosestHit(int closestHit) { dGeomRaySetClosestHit(_id, closestHit); }
