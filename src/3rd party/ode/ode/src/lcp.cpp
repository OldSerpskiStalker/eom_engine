...
static void swapRowsAndCols(ATYPE A, int n, int i1, int i2, int nskip, int do_fast_row_swaps)
{
    int i;
    dIASSERT(A && n > 0 && i1 >= 0 && i2 >= 0 && i1 < n && i2 < n && nskip >= n && i1 < i2);
...
static void swapProblem(ATYPE A, dReal* x, dReal* b, dReal* w, dReal* lo, dReal* hi, int* p, int* state, int* findex,
    int n, int i1, int i2, int nskip, int do_fast_row_swaps)
{
    dReal tmp;
    int tmpi;
...
static void checkFactorization(ATYPE A, dReal* _L, dReal* _d, int nC, int* C, int nskip)
{
    int i, j;
    if (nC == 0)
        return;
...
struct dLCP
{
    ...
    dLCP(int _n, int _nub, dReal* _Adata, dReal* _x, dReal* _b, dReal* _w, dReal* _lo, dReal* _hi, dReal* _L, dReal* _d,
        dReal* _Dell, dReal* _ell, dReal* _tmp, int* _state, int* _findex, int* _p, int* _C, dReal** Arows);
...
dLCP::dLCP(int _n, int _nub, dReal* _Adata, dReal* _x, dReal* _b, dReal* _w, dReal* _lo, dReal* _hi, dReal* _L,
    dReal* _d, dReal* _Dell, dReal* _ell, dReal* _tmp, int* _state, int* _findex, int* _p, int* _C, dReal** Arows)
{
...
dReal dLCP::Aii(int i) { return AROW(i)[i]; }
...
void dSolveLCPBasic(int n, dReal* A, dReal* x, dReal* b, dReal* w, int nub, dReal* lo, dReal* hi)
{
        ...
        else
        {
            ...
            for (;;)
            {
                ...
                for (k = nub; k < lcp.numC(); k++)
                {
                    ...
                    {
                        dReal s2 = (lo[lcp.indexC(k)] - x[lcp.indexC(k)]) / delta_x[lcp.indexC(k)];
                        if (s2 < s)
                        {
                            s = s2;
                            cmd = 5;
                    ...
                    {
                        dReal s2 = (hi[lcp.indexC(k)] - x[lcp.indexC(k)]) / delta_x[lcp.indexC(k)];
                        if (s2 < s)
                        {
                            s = s2;
                            cmd = 6;
    ...
    for (int count = 0; count < 1000; count++)
    {
        ...
        */
        ...
        for (i = 0; i < n; i++)
        {
            ...
            {
                dDebug(0, "FAILED: i=%d x=%.4e w=%.4e lo=%.4e hi=%.4e", i, x[i], w[i], lo[i], hi[i]);
            }
        }
