...
double dTimerResolution() { return 1.0 / dTimerTicksPerSecond(); }
...
{
    double ret;
    asm volatile("fildll %1; fstpl %0" : "=m"(ret) : "m"(a[0]) : "cc", "memory");
    return ret;
}

double dTimerResolution() { return 1.0 / PENTIUM_HZ; }

double dTimerTicksPerSecond() { return PENTIUM_HZ; }
...
static inline double loadClockCount(unsigned long a[2]) { return a[1] * 1.0e6 + a[0]; }
...
double dTimerTicksPerSecond() { return 1000000; }
...
double dStopwatchTime(dStopwatch* s) { return s->time / dTimerTicksPerSecond(); }
