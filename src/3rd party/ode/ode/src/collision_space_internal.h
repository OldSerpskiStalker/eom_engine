...
#define CHECK_NOT_LOCKED(space)                                                                                        \
    dUASSERT((space) == 0 || (space)->lock_count == 0, "invalid operation for locked space");
...
static void collideAABBs(dxGeom* g1, dxGeom* g2, void* data, dNearCallback* callback)
{
    ...
    if (bounds1[0] > bounds2[1] || bounds1[1] < bounds2[0] || bounds1[2] > bounds2[3] || bounds1[3] < bounds2[2] ||
