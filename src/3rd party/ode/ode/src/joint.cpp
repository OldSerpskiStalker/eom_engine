...
static inline void setBall(dxJoint* joint, dxJoint::Info2* info, dVector3 anchor1, dVector3 anchor2)
{
    ...
    if (joint->node[1].body)
    {
        for (int j = 0; j < 3; j++)
        {
            dReal err = a2[j] + joint->node[1].body->pos[j] - a1[j] - joint->node[0].body->pos[j];
    ...
    else
    {
        for (int j = 0; j < 3; j++)
        {
            info->c[j] = k * (anchor2[j] - a1[j] - joint->node[0].body->pos[j]);
        }
    }
}
...
static inline void setBall2(
    dxJoint* joint, dxJoint::Info2* info, dVector3 anchor1, dVector3 anchor2, dVector3 axis, dReal erp1)
{
    // anchor points in global coordinates with respect to body PORs.
    dVector3 a1, a2;
...
static void setAnchors(dxJoint* j, dReal x, dReal y, dReal z, dVector3 anchor1, dVector3 anchor2)
{
    if (j->node[0].body)
    {
        dReal q[4];
...
static void setAxes(dxJoint* j, dReal x, dReal y, dReal z, dVector3 axis1, dVector3 axis2)
{
    if (j->node[0].body)
    {
        dReal q[4];
...
static dReal getHingeAngle(dxBody* body1, dxBody* body2, dVector3 axis, dQuaternion q_initial)
{
...
void dxJointLimitMotor::set(int num, dReal value)
{
    ...
    case dParamHiStop:
        if (value >= lostop)
            histop = value;
        break;
    case dParamVel: vel = value; break;
    case dParamFMax:
        if (value >= 0)
            fmax = value;
        break;
    case dParamFudgeFactor:
        if (value >= 0 && value <= 1)
            fudge_factor = value;
        break;
    case dParamBounce: bounce = value; break;
    case dParamCFM: normal_cfm = value; break;
    case dParamStopERP: stop_erp = value; break;
    case dParamStopCFM: stop_cfm = value; break;
    }
}
...
int dxJointLimitMotor::addLimot(dxJoint* joint, dxJoint::Info2* info, int row, dVector3 ax1, int rotational)
{
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (joint->node[1].body)
                    {
                        ...
                        dBodyAddTorque(joint->node[0].body, -fm * ltd[0], -fm * ltd[1], -fm * ltd[2]);
...
dxJoint::Vtable __dball_vtable = {sizeof(dxJointBall), (dxJoint::init_fn*)ballInit, (dxJoint::getInfo1_fn*)ballGetInfo1,
    (dxJoint::getInfo2_fn*)ballGetInfo2, dJointTypeBall
#ifdef DE_PADF_INTEGRATION
    ,
...
static void hingeGetInfo1(dxJointHinge* j, dxJoint::Info1* info)
{
    ...
    if ((j->limot.lostop >= -M_PI || j->limot.histop <= M_PI) && j->limot.lostop <= j->limot.histop)
    {
        dReal angle = getHingeAngle(j->node[0].body, j->node[1].body, j->axis1, j->qrel);
...
extern "C" void dJointSetHingeAnchor(dxJointHinge* joint, dReal x, dReal y, dReal z)
{
    dUASSERT(joint, "bad joint argument");
...
extern "C" void dJointSetHingeAxis(dxJointHinge* joint, dReal x, dReal y, dReal z)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__dhinge_vtable, "joint is not a hinge");
...
extern "C" void dJointSetHingeParam(dxJointHinge* joint, int parameter, dReal value)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__dhinge_vtable, "joint is not a hinge");
...
dxJoint::Vtable __dhinge_vtable = {sizeof(dxJointHinge), (dxJoint::init_fn*)hingeInit,
    (dxJoint::getInfo1_fn*)hingeGetInfo1, (dxJoint::getInfo2_fn*)hingeGetInfo2, dJointTypeHinge
#ifdef DE_PADF_INTEGRATION
    ,
    (dxJoint::addBodiesForces_fn*)emptyAddForces
#endif
...
extern "C" dReal dJointGetSliderPosition(dxJointSlider* joint)
{
    ...
    if (joint->node[1].body)
    {
        ...
        for (int i = 0; i < 3; i++)
            q[i] = joint->node[0].body->pos[i] - q[i] - joint->node[1].body->pos[i];
    }
    else
    {
        for (int i = 0; i < 3; i++)
...
extern "C" dReal dJointGetSliderPositionRate(dxJointSlider* joint)
{
    ...
    if (joint->node[1].body)
    {
        return dDOT(ax1, joint->node[0].body->lvel) - dDOT(ax1, joint->node[1].body->lvel);
    }
    else
    {
...
static void sliderGetInfo1(dxJointSlider* j, dxJoint::Info1* info)
{
    ...
    // see if we're at a joint limit.
    j->limot.limit = 0;
    if ((j->limot.lostop > -dInfinity || j->limot.histop < dInfinity) && j->limot.lostop <= j->limot.histop)
    {
...
extern "C" void dJointSetSliderAxis(dxJointSlider* joint, dReal x, dReal y, dReal z)
{
    int i;
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__dslider_vtable, "joint is not a slider");
...
extern "C" void dJointSetSliderParam(dxJointSlider* joint, int parameter, dReal value)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__dslider_vtable, "joint is not a slider");
    joint->limot.set(parameter, value);
...
dxJoint::Vtable __dslider_vtable = {sizeof(dxJointSlider), (dxJoint::init_fn*)sliderInit,
    (dxJoint::getInfo1_fn*)sliderGetInfo1, (dxJoint::getInfo2_fn*)sliderGetInfo2, dJointTypeSlider
#ifdef DE_PADF_INTEGRATION
    ,
    (dxJoint::addBodiesForces_fn*)emptyAddForces
#endif
...
dxJoint::Vtable __dcontact_vtable = {sizeof(dxJointContact), (dxJoint::init_fn*)contactInit,
    (dxJoint::getInfo1_fn*)contactGetInfo1, (dxJoint::getInfo2_fn*)contactGetInfo2, dJointTypeContact
#ifdef DE_PADF_INTEGRATION
    ,
...
dxJoint::Vtable __dcontact_special_vtable = {sizeof(dxJointContact), (dxJoint::init_fn*)contactInit,
    (dxJoint::getInfo1_fn*)contactGetInfo1, (dxJoint::getInfo2_fn*)contactSpecialGetInfo2, dJointTypeContact
#ifdef DE_PADF_INTEGRATION
    ,
    (dxJoint::addBodiesForces_fn*)contactAddForces
#endif
...
static void hinge2GetInfo1(dxJointHinge2* j, dxJoint::Info1* info)
{
    ...
    int atlimit = 0;
    if ((j->limot1.lostop >= -M_PI || j->limot1.histop <= M_PI) && j->limot1.lostop <= j->limot1.histop)
    {
...
static void makeHinge2V1andV2(dxJointHinge2* joint)
{
    if (joint->node[0].body)
    {
        ...
        if ((ax1[0] == 0 && ax1[1] == 0 && ax1[2] == 0) || (ax2[0] == 0 && ax2[1] == 0 && ax2[2] == 0) ||
...
extern "C" void dJointSetHinge2Anchor(dxJointHinge2* joint, dReal x, dReal y, dReal z)
{
    dUASSERT(joint, "bad joint argument");
...
extern "C" void dJointSetHinge2Axis1(dxJointHinge2* joint, dReal x, dReal y, dReal z)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__dhinge2_vtable, "joint is not a hinge2");
    if (joint->node[0].body)
...
extern "C" void dJointSetHinge2Axis2(dxJointHinge2* joint, dReal x, dReal y, dReal z)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__dhinge2_vtable, "joint is not a hinge2");
    if (joint->node[1].body)
...
extern "C" void dJointSetHinge2Param(dxJointHinge2* joint, int parameter, dReal value)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__dhinge2_vtable, "joint is not a hinge2");
...
dxJoint::Vtable __dhinge2_vtable = {sizeof(dxJointHinge2), (dxJoint::init_fn*)hinge2Init,
    (dxJoint::getInfo1_fn*)hinge2GetInfo1, (dxJoint::getInfo2_fn*)hinge2GetInfo2, dJointTypeHinge2
#ifdef DE_PADF_INTEGRATION
    ,
    (dxJoint::addBodiesForces_fn*)emptyAddForces
#endif
...
static void universalGetInfo1(dxJointUniversal* j, dxJoint::Info1* info)
{
    ...
    bool limiting1 = (j->limot1.lostop >= -M_PI || j->limot1.histop <= M_PI) && j->limot1.lostop <= j->limot1.histop;
    bool limiting2 = (j->limot2.lostop >= -M_PI || j->limot2.histop <= M_PI) && j->limot2.lostop <= j->limot2.histop;
...
extern "C" void dJointSetUniversalAnchor(dxJointUniversal* joint, dReal x, dReal y, dReal z)
{
    dUASSERT(joint, "bad joint argument");
...
extern "C" void dJointSetUniversalAxis1(dxJointUniversal* joint, dReal x, dReal y, dReal z)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__duniversal_vtable, "joint is not a universal");
    if (joint->flags & dJOINT_REVERSE)
...
extern "C" void dJointSetUniversalAxis2(dxJointUniversal* joint, dReal x, dReal y, dReal z)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__duniversal_vtable, "joint is not a universal");
    if (joint->flags & dJOINT_REVERSE)
...
extern "C" void dJointGetUniversalAnchor2(dxJointUniversal* joint, dVector3 result)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(result, "bad result argument");
    dUASSERT(joint->vtable == &__duniversal_vtable, "joint is not a universal");
...
extern "C" void dJointSetUniversalParam(dxJointUniversal* joint, int parameter, dReal value)
{
    dUASSERT(joint, "bad joint argument");
    dUASSERT(joint->vtable == &__duniversal_vtable, "joint is not a universal");
    if ((parameter & 0xff00) == 0x100)
...
dxJoint::Vtable __duniversal_vtable = {sizeof(dxJointUniversal), (dxJoint::init_fn*)universalInit,
    (dxJoint::getInfo1_fn*)universalGetInfo1, (dxJoint::getInfo2_fn*)universalGetInfo2, dJointTypeUniversal
#ifdef DE_PADF_INTEGRATION
    ,
    (dxJoint::addBodiesForces_fn*)emptyAddForces
#endif
...
static void amotorGetInfo1(dxJointAMotor* j, dxJoint::Info1* info)
{
    ...
    {
        if (j->limot[i].testRotationalLimit(j->angle[i]) || j->limot[i].fmax > 0)
        {
            info->m++;
        }
    }
...
extern "C" void dJointSetAMotorAxis(dxJointAMotor* joint, int anum, int rel, dReal x, dReal y, dReal z)
{
    dAASSERT(joint && anum >= 0 && anum <= 2 && rel >= 0 && rel <= 2);
    dUASSERT(joint->vtable == &__damotor_vtable, "joint is not an amotor");
    dUASSERT(
        !(!joint->node[1].body && (joint->flags & dJOINT_REVERSE) && rel == 1), "no first body, can't set axis rel=1");
    dUASSERT(!(!joint->node[1].body && !(joint->flags & dJOINT_REVERSE) && rel == 2),
...
extern "C" void dJointSetAMotorParam(dxJointAMotor* joint, int parameter, dReal value)
{
    dAASSERT(joint);
    dUASSERT(joint->vtable == &__damotor_vtable, "joint is not an amotor");
...
extern "C" void dJointGetAMotorAxis(dxJointAMotor* joint, int anum, dVector3 result)
{
    dAASSERT(joint && anum >= 0 && anum < 3);
    dUASSERT(joint->vtable == &__damotor_vtable, "joint is not an amotor");
    if (anum < 0)
...
extern "C" void dJointAddAMotorTorques(dxJointAMotor* joint, dReal torque1, dReal torque2, dReal torque3)
{
    ...
    if (joint->num == 0)
        return;
    dUASSERT(
        (joint->flags & dJOINT_REVERSE) == 0, "dJointAddAMotorTorques not yet implemented for reverse AMotor joints");
...
dxJoint::Vtable __damotor_vtable = {sizeof(dxJointAMotor), (dxJoint::init_fn*)amotorInit,
    (dxJoint::getInfo1_fn*)amotorGetInfo1, (dxJoint::getInfo2_fn*)amotorGetInfo2, dJointTypeAMotor
#ifdef DE_PADF_INTEGRATION
    ,
    (dxJoint::addBodiesForces_fn*)emptyAddForces
#endif
...
static void fixedGetInfo2(dxJointFixed* joint, dxJoint::Info2* info)
{
    ...
    {
        for (int j = 0; j < 3; j++)
            info->c[j] = k * (joint->node[1].body->pos[j] - joint->node[0].body->pos[j] + ofs[j]);
    }
    else
    {
        for (int j = 0; j < 3; j++)
...
extern "C" void dJointSetFixedQuaternionPos(dxJointFixed* joint, dQuaternion quaternion, dReal* pos)
{
    ...
    if (joint->node[0].body)
    {
        ...
        else
        {
            ...
            for (i = 1; i < 4; i++)
                joint->qrel[i] = -quaternion[i]; //-joint->node[0].body->q[i];
            for (i = 0; i < 4; i++)
...
dxJoint::Vtable __dfixed_vtable = {sizeof(dxJointFixed), (dxJoint::init_fn*)fixedInit,
    (dxJoint::getInfo1_fn*)fixedGetInfo1, (dxJoint::getInfo2_fn*)fixedGetInfo2, dJointTypeFixed
#ifdef DE_PADF_INTEGRATION
    ,
    (dxJoint::addBodiesForces_fn*)emptyAddForces
#endif
...
static void nullGetInfo2(dxJointNull* joint, dxJoint::Info2* info) { dDebug(0, "this should never get called"); }

dxJoint::Vtable __dnull_vtable = {sizeof(dxJointNull), (dxJoint::init_fn*)0, (dxJoint::getInfo1_fn*)nullGetInfo1,
    (dxJoint::getInfo2_fn*)nullGetInfo2, dJointTypeNull
#ifdef DE_PADF_INTEGRATION
    ,
    (dxJoint::addBodiesForces_fn*)emptyAddForces
#endif
