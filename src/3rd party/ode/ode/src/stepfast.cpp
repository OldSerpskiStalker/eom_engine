...
int dWorldGetAutoEnableDepthSF1(dxWorld* world) { return autoEnableDepth; }

// little bit of math.... the _sym_ functions assume the return matrix will be symmetric
static void Multiply2_sym_p8p(dReal* A, dReal* B, dReal* C, int p, int Askip)
{
    int i, j;
...
static void MultiplyAdd2_sym_p8p(dReal* A, dReal* B, dReal* C, int p, int Askip)
{
    int i, j;
    dReal sum, *aa, *ad, *bb, *cc;
    dIASSERT(p > 0 && A && B && C);
...
void dInternalStepFast(dxWorld* world, dxBody* body[2], dReal* GI[2], dReal* GinvI[2], dxJoint* joint,
    dxJoint::Info1 info, dxJoint::Info2 Jinfo, dReal stepsize)
{
    int i, j, k;
#ifdef TIMING
...
void dInternalStepIslandFast(
    dxWorld* world, dxBody* const* bodies, int nb, dxJoint** joints, int nj, dReal stepsize, int maxiterations)
{
#ifdef TIMING
                ...
                case dJointTypeContact:
                    if (info[j].m == 3)
                    {
                        dInternalStepJointContact(
                            world, bodyPair, GIPair, GinvIPair, joint, info[j], Jinfo[j], ministep);
...
static void processIslandsFast(dxWorld* world, dReal stepsize, int maxiterations)
{
