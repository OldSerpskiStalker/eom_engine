...
void dWorldCheck(dxWorld* w) { checkWorld(w); }
...
{
    dAASSERT(c);
    dxJointContact* j = (dxJointContact*)createJoint(w, group, &__dcontact_vtable);
    j->contact = *c;
    return j;
}
dxJoint* dJointCreateContactSpecial(dWorldID w, dJointGroupID group, const dContact* c)
{
    dAASSERT(c);
    dxJointContact* j = (dxJointContact*)createJoint(w, group, &__dcontact_special_vtable);
    j->contact = *c;
    return j;
}
