...
void dxSpace::setCleanup(int mode) { cleanup = (mode != 0); }

int dxSpace::getCleanup() { return cleanup; }
...
int dxSpace::getNumGeoms() { return count; }
...
void dxSpace::add(dxGeom* geom)
{
    CHECK_NOT_LOCKED(this);
    dAASSERT(geom);
    dUASSERT(geom->parent_space == 0 && geom->next == 0, "geom is already in a space");
...
dxSimpleSpace::dxSimpleSpace(dSpaceID _space) : dxSpace(_space) { type = dSimpleSpaceClass; }
...
void dxSimpleSpace::collide2(void* data, dxGeom* geom, dNearCallback* callback)
{
...
static long int prime[NUM_PRIMES] = {1L, 2L, 3L, 7L, 13L, 31L, 61L, 127L, 251L, 509L, 1021L, 2039L, 4093L, 8191L,
    16381L, 32749L, 65521L, 131071L, 262139L, 524287L, 1048573L, 2097143L, 4194301L, 8388593L, 16777213L, 33554393L,
...
{
    if (bounds[0] <= -dInfinity || bounds[1] >= dInfinity || bounds[2] <= -dInfinity || bounds[3] >= dInfinity ||
...
static unsigned long getVirtualAddress(int level, int x, int y, int z) { return level * 1000 + x * 100 + y * 10 + z; }
...
void dxHashSpace::collide2(void* data, dxGeom* geom, dNearCallback* callback)
{
    dAASSERT(geom && callback);
...
dxSpace* dSimpleSpaceCreate(dxSpace* space) { return new dxSimpleSpace(space); }

dxSpace* dHashSpaceCreate(dxSpace* space) { return new dxHashSpace(space); }
