...
static void compute_invM_JT(int m, dRealMutablePtr J, dRealMutablePtr iMJ, int* jb, dxBody* const* body, dRealPtr invI)
{
    int i, j;
...
static void multiply_invM_JT(int m, int nb, dRealMutablePtr iMJ, int* jb, dRealMutablePtr in, dRealMutablePtr out)
{
    int i, j;
    dSetZero(out, 6 * nb);
...
static void multiply_J(int m, dRealMutablePtr J, int* jb, dRealMutablePtr in, dRealMutablePtr out)
{
    int i, j;
    dRealPtr J_ptr = J;
    for (i = 0; i < m; i++)
...
static void SOR_LCP(int m, int nb, dRealMutablePtr J, int* jb, dxBody* const* body, dRealPtr invI,
    dRealMutablePtr lambda, dRealMutablePtr fc, dRealMutablePtr b, dRealMutablePtr lo, dRealMutablePtr hi, dRealPtr cfm,
    int* findex, dxQuickStepParameters* qs)
{
    const int num_iterations = qs->num_iterations;
    ...
    for (int iteration = 0; iteration < num_iterations; iteration++)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            delta -= fc_ptr[0] * J_ptr[0] + fc_ptr[1] * J_ptr[1] + fc_ptr[2] * J_ptr[2] + fc_ptr[3] * J_ptr[3] +
            ...
            {
                fc_ptr = fc + 6 * b2;
                delta -= fc_ptr[0] * J_ptr[6] + fc_ptr[1] * J_ptr[7] + fc_ptr[2] * J_ptr[8] + fc_ptr[3] * J_ptr[9] +
...
void dxQuickStepper(dxWorld* world, dxBody* const* body, int nb, dxJoint** joint, int nj, dReal stepsize)
{
    int i, j;
    ...
    // dxJoint **joint = (dxJoint**) alloca (nj * sizeof(dxJoint*));//slipch@ - no need  to copy with sipmle island
    ...
    for (i = 0; i < nb; i++)
    {
        ...
        if (!(dValid(b->tacc[0]) && dValid(b->tacc[1]) && dValid(b->tacc[2])))
        {
            ...
            //%f,%f,%f",tmp[0],tmp[1],tmp[2],b->avel[0],b->avel[1],b->avel[2]); dUASSERT(0,"tmp %f,%f,%f \n avel
            // %f,%f,%f",tmp[0],tmp[1],tmp[2],b->avel[0],b->avel[1],b->avel[2]);

            dDebug(d_ERR_UASSERT, " (%s:%d \n tmp %f,%f,%f \n avel %f,%f,%f)", __FILE__, __LINE__, tmp[0], tmp[1],
                tmp[2], b->avel[0], b->avel[1], b->avel[2]);
        }
