    ...
    if (Depth > 0)
    {
        ...
        for (int i = 0; i < SPLITAXIS; i++)
        {
            for (int j = 0; j < SPLITAXIS; j++)
            {
                ...
                ChildCenter[AXIS0] =
                    Center[AXIS0] - Extents[AXIS0] + ChildExtents[AXIS0] + i * (ChildExtents[AXIS0] * 2);
                ChildCenter[AXIS1] =
                    Center[AXIS1] - Extents[AXIS1] + ChildExtents[AXIS1] + j * (ChildExtents[AXIS1] * 2);
...
void Block::Collide(dxGeom* g1, dxGeom* g2, void* UserData, dNearCallback* Callback)
{
    ...
    if (Children)
    {
        for (int i = 0; i < SPLITS; i++)
        {
            ...
            {
                if (g1->aabb[AXIS0 * 2 + 0] > Children[i].MaxX || g1->aabb[AXIS0 * 2 + 1] < Children[i].MinX ||
                    g1->aabb[AXIS1 * 2 + 0] > Children[i].MaxZ || g1->aabb[AXIS1 * 2 + 1] < Children[i].MinZ)
                    continue;
            }
...
{
    return AABB[AXIS0 * 2 + 0] >= MinX && AABB[AXIS0 * 2 + 1] <= MaxX && AABB[AXIS1 * 2 + 0] >= MinZ &&
        AABB[AXIS1 * 2 + 1] <= MaxZ;
}
...
dxQuadTreeSpace::dxQuadTreeSpace(dSpaceID _space, dVector3 Center, dVector3 Extents, int Depth) : dxSpace(_space)
{
    ...
    // Init AABB. We initialize to infinity because it is not illegal for an object to be outside of the tree. Its
    // simply inserted in the root block
    aabb[0] = -dInfinity;
    aabb[1] = dInfinity;
    aabb[2] = -dInfinity;
    aabb[3] = dInfinity;
...
void dxQuadTreeSpace::dirty(dxGeom* g) { DirtyList.push(g); }
