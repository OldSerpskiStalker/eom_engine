...
TEMPLATE_SPECIALIZATION
typename MEMORY_ALLOCATOR::pointer MEMORY_ALLOCATOR::address(reference value) const { return (&value); }

TEMPLATE_SPECIALIZATION
typename MEMORY_ALLOCATOR::const_pointer MEMORY_ALLOCATOR::address(const_reference value) const { return (&value); }
...
TEMPLATE_SPECIALIZATION
char* MEMORY_ALLOCATOR::__charalloc(size_type const n) { return ((char _FARQ*)allocate(n)); }

TEMPLATE_SPECIALIZATION
void MEMORY_ALLOCATOR::deallocate(pointer const p, size_type const n) const { call_allocator(p, 0); }

TEMPLATE_SPECIALIZATION
void MEMORY_ALLOCATOR::deallocate(void* p, size_type n) const { call_allocator(p, 0); }

TEMPLATE_SPECIALIZATION
void MEMORY_ALLOCATOR::construct(pointer const p, T const& value) { new (p) T(value); }

TEMPLATE_SPECIALIZATION
void MEMORY_ALLOCATOR::destroy(pointer const p) { p->~T(); }
