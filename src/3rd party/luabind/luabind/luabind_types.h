...
{
public:
    inline unsigned int size() const { return (unsigned int)(std::vector<T, memory_allocator<T>>::size()); }
};
template <typename T>
...
class internal_map : public std::map<K, V, P, memory_allocator<std::pair<const K, V>>>
{
public:
};
template <typename K, class V, class P = std::less<K>>
class internal_multimap : public std::multimap<K, V, P, memory_allocator<std::pair<const K, V>>>
{
public:
};
