...
{
    static inline void apply(T*& pointer) { delete_helper2(pointer, dynamic_cast<void*>(pointer)); }
};

template <typename T>
struct delete_helper<T, false>
{
    static inline void apply(T*& pointer) { delete_helper2(pointer, pointer); }
};
...
inline void luabind_delete(T*& pointer)
{
    ...
    delete_helper<T, boost::is_polymorphic<T>::value>::apply(pointer);
}
} // namespace luabind
