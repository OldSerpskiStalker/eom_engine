 ...
 *
 *  \param rplColor : input color (returned for out-of-bound coordinates in OF_COLOR mode and if other mode is not
 * applicable)
 *
...
RGBQUAD CxImage::GetPixelColorWithOverflow(long x, long y, OverflowMethod const ofMethod, RGBQUAD* const rplColor)
{
    ...
    { // is pixel within bouns?:
       ...
       // no alpha transparency
        ...
        case OM_REPEAT:
        case OM_WRAP:
        case OM_MIRROR: OverflowCoordinates(x, y, ofMethod); break;
        default:
...
RGBQUAD CxImage::GetPixelColorInterpolated(
    float x, float y, InterpolationMethod const inMethod, OverflowMethod const ofMethod, RGBQUAD* const rplColor)
{
    ...
    switch (inMethod)
    {
    case IM_NEAREST_NEIGHBOUR: return GetPixelColorWithOverflow((long)(x + 0.5f), (long)(y + 0.5f), ofMethod, rplColor);
    default: {
...
#if CXIMAGE_SUPPORT_ALPHA
            if (AlphaIsValid())
                color.rgbReserved = (BYTE)(a * rgb11.rgbReserved + b * rgb21.rgbReserved + c * rgb12.rgbReserved +
                    d * rgb22.rgbReserved);
            else
#endif
            { // Alpha not supported or no alpha at all
    ...
    case IM_LANCZOS:
        ...
        if (((xi + 6) < head.biWidth) && ((xi - 5) >= 0) && ((yi + 6) < head.biHeight) && ((yi - 5) >= 0) &&
            !IsIndexed())
        {
            // optimized interpolation (faster pixel reads) for RGB24 images with all pixels inside bounds
            BYTE *pxptr, *pxptra;
 ...
 * \author ***bd*** 2.2004
 */
RGBQUAD CxImage::GetAreaColorInterpolated(float const xc, float const yc, float const w, float const h,
    InterpolationMethod const inMethod, OverflowMethod const ofMethod, RGBQUAD* const rplColor)
{
    RGBQUAD color; // calculated colour
    ...
    else
    {
        ...
        if (h > 1 && w > 1)
        {
            ...
            intBL = area.CrossSection(
                CxRect2(((float)xi1) - 0.5f, ((float)yi1) - 0.5f, ((float)xi1) + 0.5f, ((float)yi1) + 0.5f));
            intTR = area.CrossSection(
                CxRect2(((float)xi2) - 0.5f, ((float)yi2) - 0.5f, ((float)xi2) + 0.5f, ((float)yi2) + 0.5f));
            ...
            AddAveragingCont(GetPixelColorWithOverflow(xi1, yi1, ofMethod, rplColor), wBL * hBL, rr, gg, bb,
                aa); // bottom left pixel
            AddAveragingCont(GetPixelColorWithOverflow(xi2, yi1, ofMethod, rplColor), wTR * hBL, rr, gg, bb,
                aa); // bottom right pixel
            AddAveragingCont(
                GetPixelColorWithOverflow(xi1, yi2, ofMethod, rplColor), wBL * hTR, rr, gg, bb, aa); // top left pixel
            AddAveragingCont(
                GetPixelColorWithOverflow(xi2, yi2, ofMethod, rplColor), wTR * hTR, rr, gg, bb, aa); // top right pixel
            // bottom and top row
            for (x = xi1 + 1; x < xi2; x++)
            {
                AddAveragingCont(
                    GetPixelColorWithOverflow(x, yi1, ofMethod, rplColor), hBL, rr, gg, bb, aa); // bottom row
            ...
            {
                AddAveragingCont(
                    GetPixelColorWithOverflow(xi1, y, ofMethod, rplColor), wBL, rr, gg, bb, aa); // left column
                AddAveragingCont(
                    GetPixelColorWithOverflow(xi2, y, ofMethod, rplColor), wTR, rr, gg, bb, aa); // right column
            }
            for (y = yi1 + 1; y < yi2; y++)
            {
                for (x = xi1 + 1; x < xi2; x++)
        ...
        else
        {
            ...
            for (y = yi1; y <= yi2; y++)
            {
                for (x = xi1; x <= xi2; x++)
                {
                    intersect = area.CrossSection(
                        CxRect2(((float)x) - 0.5f, ((float)y) - 0.5f, ((float)x) + 0.5f, ((float)y) + 0.5f));
...
float CxImage::KernelBessel_J1(const float x)
{
    ...
    static const double Pone[] = {0.581199354001606143928050809e+21, -0.6672106568924916298020941484e+20,
        0.2316433580634002297931815435e+19, -0.3588817569910106050743641413e+17, 0.2908795263834775409737601689e+15,
        -0.1322983480332126453125473247e+13, 0.3413234182301700539091292655e+10, -0.4695753530642995859767162166e+7,
...
float CxImage::KernelBessel_P1(const float x)
{
    ...
    static const double Pone[] = {0.352246649133679798341724373e+5, 0.62758845247161281269005675e+5,
        0.313539631109159574238669888e+5, 0.49854832060594338434500455e+4, 0.2111529182853962382105718e+3,
...
float CxImage::KernelBessel_Q1(const float x)
{
    ...
    static const double Pone[] = {0.3511751914303552822533318e+3, 0.7210391804904475039280863e+3,
        0.4259873011654442389886993e+3, 0.831898957673850827325226e+2, 0.45681716295512267064405e+1,
...
float CxImage::KernelBessel_Order1(float x)
{
    ...
    q = (float)sqrt(2.0f / (PI * x)) *
        (float)(KernelBessel_P1(x) * (1.0f / sqrt(2.0f) * (sin(x) - cos(x))) -
