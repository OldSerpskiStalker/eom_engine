 ...
 */
bool CxImage::SelectionIsValid() { return pSelection != 0; }
////////////////////////////////////////////////////////////////////////////////
/**
 * Gets the smallest rectangle that contains the selection
 */
void CxImage::SelectionGetBox(RECT& r) { memcpy(&r, &info.rSelectionBox, sizeof(RECT)); }
////////////////////////////////////////////////////////////////////////////////
/**
...
{
    if (from.pSelection == NULL || head.biWidth != from.head.biWidth || head.biHeight != from.head.biHeight)
        return false;
    if (pSelection == NULL)
        pSelection = (BYTE*)cxalloc(head.biWidth * head.biHeight); // malloc(head.biWidth * head.biHeight);
    if (pSelection == NULL)
        return false;
...
bool CxImage::SelectionAddPolygon(POINT* points, long npoints, BYTE level)
{
    ...
    while (i < npoints)
    {
        ...
        if (localbox.bottom > r2.bottom)
            localbox.bottom = max(0L, min(head.biHeight - 1, r2.bottom - 1));
            ...
            for (x = xmin; x < xmax; x++)
            {
                if (plocal[x + y * head.biWidth] == 0)
                {
                    ...
                    while (first != last)
                    {
                        ...
                        for (;;)
                        {
                            if ((plocal[fxx + fyy * head.biWidth] == back) && fxx >= localbox.left &&
                                fxx <= localbox.right && fyy >= localbox.bottom && fyy <= localbox.top)
                            {
...
bool CxImage::SelectionAddColor(RGBQUAD c, BYTE level)
{
    ...
    for (long y = 0; y < head.biHeight; y++)
    {
        for (long x = 0; x < head.biWidth; x++)
        {
            ...
            if (color.rgbRed == c.rgbRed && color.rgbGreen == c.rgbGreen && color.rgbBlue == c.rgbBlue)
            {
                pSelection[x + y * head.biWidth] = level;
