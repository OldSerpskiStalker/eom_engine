...
Image* nv::ImageIO::loadTGA(Stream& s)
{
    ...
    case TGA_TYPE_RLE_RGB:
        rle = true;
        // no break is intended!
    case TGA_TYPE_RGB: rgb = true; break;

    case TGA_TYPE_RLE_GREY:
        rle = true;
        // no break is intended!
    case TGA_TYPE_GREY: grey = true; break;

    default: nvDebug("*** ImageIO::loadTGA: Error, unsupported image type.\n"); return false;
    }
...
FloatImage* nv::ImageIO::loadFloatTIFF(const char* fileName, Stream& s)
{
    ...
    // TIFF * tif = TIFFClientOpen(fileName, "r", &s, tiffReadWriteProc, tiffReadWriteProc, tiffSeekProc, tiffCloseProc,
...
bool nv::ImageIO::saveFloatTIFF(
    const char* fileName, const FloatImage* fimage, uint base_component, uint num_components)
{
...
{
public:
    ExrStream(const char* name, Stream& s) : Imf::IStream(name), m_stream(s) { nvDebugCheck(s.isLoading()); }
...
FloatImage* nv::ImageIO::loadFloatEXR(const char* fileName, Stream& s)
{
    ...
    {
        frameBuffer.insert(
            it.name(), Imf::Slice(Imf::FLOAT, (char*)fimage->channel(i), sizeof(float), sizeof(float) * width));
    }
