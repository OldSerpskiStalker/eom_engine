...
// Simple quantization.
void nv::Quantize::RGB16(Image* image) { Truncate(image, 5, 6, 5, 8); }

// Alpha quantization.
void nv::Quantize::Alpha4(Image* image) { Truncate(image, 8, 8, 8, 4); }

// Error diffusion. Floyd Steinberg.
void nv::Quantize::FloydSteinberg_RGB16(Image* image) { FloydSteinberg(image, 5, 6, 5, 8); }
...
// Error diffusion. Floyd Steinberg.
void nv::Quantize::FloydSteinberg_Alpha4(Image* image) { FloydSteinberg(image, 8, 8, 8, 4); }
...
void nv::Quantize::FloydSteinberg(Image* image, uint rsize, uint gsize, uint bsize, uint asize)
{
    ...
    for (uint y = 0; y < h; y++)
    {
        for (uint x = 0; x < w; x++)
        {
            ...
            // Compute new error.
            Vector4 diff(
                float(int(pixel.r) - r), float(int(pixel.g) - g), float(int(pixel.b) - b), float(int(pixel.a) - a));
