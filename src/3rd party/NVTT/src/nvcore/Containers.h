...
#define NV_FOREACH(i, container)                                                                                       \
    ...
    for (NV_STRING_JOIN2(cont, __LINE__)::PseudoIndex i((container).start()); !(container).isDone(i);                  \
         (container).advance(i))
/*
#define NV_FOREACH(i, container) \
...
inline bool isPowerOfTwo(uint n) { return (n & (n - 1)) == 0; }
...
public:
    ...
    /// Copy ctor.
    Array(const Array& a) : m_buffer(NULL), m_size(0), m_buffer_size(0) { copy(a.m_buffer, a.m_size); }

    /// Ctor that initializes the vector with the given elements.
    Array(const T* ptr, int num) : m_buffer(NULL), m_size(0), m_buffer_size(0) { copy(ptr, num); }

    /// Allocate array.
    explicit Array(uint capacity) : m_buffer(NULL), m_size(0), m_buffer_size(0) { allocate(capacity); }
    ...
    void append(const Array<T>& other) { append(other.m_buffer, other.m_size); }
...
#if NV_CC_MSVC
    T& operator[](const PseudoIndexWrapper& i) { return m_buffer[i(this)]; }
    const T& operator[](const PseudoIndexWrapper& i) const { return m_buffer[i(this)]; }
#endif
...
public:
    ...
    const Entry& operator[](const PseudoIndexWrapper& i) const { return E(i(this)); }
#endif
