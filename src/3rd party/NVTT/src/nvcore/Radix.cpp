...
/**
 *	Main sort routine.
 *	This one is for integer values. After the call, mIndices contains a list of indices in sorted order, i.e. in the
 *order you may process your data. \param		input			[in] a list of integer values to sort \param		nb
 *[in] number of values to sort \param		signedvalues	[in] true to handle negative values, false if you know your
 *input buffer only contains positive values \return		Self-Reference
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadixSort& RadixSort::sort(const uint32* input, uint32 nb, bool signedvalues)
{
    ...
    for (j = 0; j < 4; j++)
    {
        ...
        // Sometimes the fourth (negative) pass is skipped because all numbers are negative and the MSB is 0xFF (for
        // example). This is not a problem, numbers are correctly sorted anyway.
        if (PerformPass)
        {
            ...
            {
                // This is a special case to correctly handle negative integers. They're sorted in the right order but
                // at the wrong place.

                // Create biased offsets, in order for negative numbers to be sorted as well
                mOffset[0] = NbNegativeValues; // First positive number takes place after the negative ones
                for (i = 1; i < 128; i++)
                    mOffset[i] = mOffset[i - 1] + CurCount[i - 1]; // 1 to 128 for positive numbers
...
/**
 *	Main sort routine.
 *	This one is for floating-point values. After the call, mIndices contains a list of indices in sorted order, i.e. in
 *the order you may process your data. \param		input			[in] a list of floating-point values to sort \param
 *nb				[in] number of values to sort \return		Self-Reference \warning	only sorts IEEE floating-point
 *values
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (j = 0; j < 4; j++)
    {
        ...
        else
        {
            ...
            if (PerformPass)
            {
                ...
                for (i = 0; i < 127; i++)
                    mOffset[254 - i] =
                        mOffset[255 - i] + CurCount[255 - i]; // Fixing the wrong order for negative values
                for (i = 128; i < 256; i++)
            ...
            {
                // The pass is useless, yet we still have to reverse the order of current list if all values are
                // negative.
                if (UniqueVal >= 128)
                {
                    for (i = 0; i < nb; i++)
                    ...
                    // Swap pointers for next pass. Valid indices - the most recent ones - are in mIndices after the
                    // swap.
                    uint32* Tmp = mIndices;
                    mIndices = mIndices2;
                    mIndices2 = Tmp;
