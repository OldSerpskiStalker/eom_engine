...
namespace
{
static char* strAlloc(uint size) { return static_cast<char*>(mem::malloc(size)); }

static char* strReAlloc(char* str, uint size) { return static_cast<char*>(mem::realloc(str, size)); }

static void strFree(const char* str) { return mem::free(const_cast<char*>(str)); }
...
static inline char toUpper(char c) { return (c < 'a' || c > 'z') ? (c) : (c + 'A' - 'a'); }
static inline char toLower(char c) { return (c < 'A' || c > 'Z') ? (c) : (c + 'a' - 'A'); }
...
/** Copy ctor. */
StringBuilder::StringBuilder(const StringBuilder& s) : m_size(0), m_str(NULL) { copy(s); }

/** Copy string. */
StringBuilder::StringBuilder(const char* s) : m_size(0), m_str(NULL) { copy(s); }
...
const char* Path::fileName() const { return fileName(m_str); }

/// Get the extension from a file path.
const char* Path::extension() const { return extension(m_str); }
...
// static
char Path::separator() { return NV_PATH_SEPARATOR; }
