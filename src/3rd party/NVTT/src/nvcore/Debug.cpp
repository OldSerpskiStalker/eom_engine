...
static LONG WINAPI nvTopLevelFilter(struct _EXCEPTION_POINTERS* pExceptionInfo)
{
    ...
    /*	BOOL (WINAPI * Dump) (HANDLE, DWORD, HANDLE, MINIDUMP_TYPE, PMINIDUMP_EXCEPTION_INFORMATION,
       PMINIDUMP_USER_STREAM_INFORMATION, PMINIDUMP_CALLBACK_INFORMATION );
        ...
        // create the file
        HANDLE hFile = ::CreateFile( "nv.dmp", GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS,
       FILE_ATTRIBUTE_NORMAL, NULL ); if( hFile == INVALID_HANDLE_VALUE ) { nvDebug("*** Failed to create dump
       file.\n"); return EXCEPTION_CONTINUE_SEARCH;
        }
...
struct Win32AssertHandler : public AssertHandler
{
    ...
    virtual int assert(const char* exp, const char* file, int line, const char* func /*=NULL*/)
    {
        ...
        switch (action)
        {
        case IDRETRY: ret = NV_ABORT_DEBUG; break;
        case IDIGNORE: ret = NV_ABORT_IGNORE; break;
        case IDABORT:
        default: ret = NV_ABORT_EXIT; break;
        }
...
void debug::setMessageHandler(MessageHandler* message_handler) { s_message_handler = message_handler; }

/// Reset the debug message handler.
void debug::resetMessageHandler() { s_message_handler = NULL; }

/// Set the assert handler.
void debug::setAssertHandler(AssertHandler* assert_handler) { s_assert_handler = assert_handler; }

/// Reset the assert handler.
void debug::resetAssertHandler() { s_assert_handler = NULL; }
