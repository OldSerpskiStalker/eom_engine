...
public:
    ...
    void cube(Vector3::Arg center, float dist) { setCenterExtents(center, Vector3(dist, dist, dist)); }
    ...
    {
        return m_mins.x() < p.x() && m_mins.y() < p.y() && m_mins.z() < p.z() && m_maxs.x() > p.x() &&
            m_maxs.y() > p.y() && m_maxs.z() > p.z();
    }
