...
char const* engine::lua_getlocal(lua_State* L, const lua_Debug* ar, int n) { return (::lua_getlocal(L, ar, n)); }
...
char const* engine::lua_getupvalue(lua_State* L, int funcindex, int n) { return (::lua_getupvalue(L, funcindex, n)); }
...
char const* engine::lua_tolstring(lua_State* L, int idx, size_t* len) { return (::lua_tolstring(L, idx, len)); }
...
char const* engine::lua_typename(lua_State* L, int t) { return (::lua_typename(L, t)); }
...
char const* engine::lua_debug_get_name(lua_Debug& instance) { return (instance.name); }

char const* engine::lua_debug_get_source(lua_Debug& instance) { return (instance.source); }

char const* engine::lua_debug_get_short_source(lua_Debug& instance) { return (instance.short_src); }
...
bool engine::type_to_string(
    char* const buffer, unsigned int const size, lua_State* const state, int const index, bool& use_in_description)
{
...
void engine::value_convert_class(cs::lua_studio::backend& backend, char* buffer, unsigned int size,
    luabind::detail::class_rep* class_rep, lua_State* state, int index, cs::lua_studio::icon_type& icon_type,
...
bool engine::value_to_string(cs::lua_studio::backend& backend, char* const buffer, unsigned int const size,
    lua_State* const state, int const index, cs::lua_studio::icon_type& icon_type, bool const full_description)
{
    switch (lua_type(state, index))
    {
    case engine::lua_type_string:
    ...
    case engine::lua_type_user_data: {
        if (!luabind::detail::is_class_object(state, index))
        {
            ...
            luabind::detail::class_rep* class_rep =
                static_cast<luabind::detail::class_rep*>(lua_touserdata(state, index));
            VERIFY(class_rep);
...
void engine::fill_class_data(
    cs::lua_studio::backend& backend, cs::lua_studio::value_to_expand& value_to_expand, lua_State* const state)
{
...
void engine::expand_class(
    cs::lua_studio::backend& backend, cs::lua_studio::value_to_expand& value, lua_State* const state)
{
