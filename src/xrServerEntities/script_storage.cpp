...
u32 game_lua_memory_usage() { return (s_allocator.get_allocated_size()); }
#endif //! USE_DL_ALLOCATOR

static LPVOID __cdecl luabind_allocator(
    luabind::memory_allocation_function_parameter const, void const* const pointer, size_t const size)
{
    if (!size)
    {
...
static void l_message(lua_State* state, const char* msg) { Msg("! [LUA_JIT] %s", msg); }
...
void jit_command(lua_State* state, LPCSTR command) { dojitcmd(state, command); }
