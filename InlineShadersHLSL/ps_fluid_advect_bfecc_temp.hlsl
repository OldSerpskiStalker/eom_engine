...
float4 main( p_fluidsim input ) : SV_Target
{
    ...
    float3 nposTC = float3(npos.x/textureWidth, npos.y/textureHeight, (npos.z+0.5f)/textureDepth);

    // find the texel corner closest to the semi-Lagrangian "particle"
    float3 nposTexel = floor( npos + float3( 0.5f, 0.5f, 0.5f ) );
    float3 nposTexelTC = float3( nposTexel.x/textureWidth, nposTexel.y/textureHeight, (nposTexel.z+0.5f)/textureDepth);
