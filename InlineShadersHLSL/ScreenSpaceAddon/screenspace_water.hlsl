...
float3 SSFX_ssr_water_ray(float3 ray_start_vs, float3 ray_dir_vs, float noise, uint iSample : SV_SAMPLEINDEX)
{
	...
	float4 prev_step;
	int prev_sign;
	float3 behind_hit = 0.f;

	float RayThick = clamp( 48.0f / q_steps[G_SSR_WATER_QUALITY].x, 1.f, 3.f);

	// Initialize Ray
	RayTrace ssr_ray = SSFX_ray_init(ray_start_vs, ray_dir_vs, 150.f, q_steps[G_SSR_WATER_QUALITY].x, noise);
	...
	for (int st = 1; st <= q_steps[G_SSR_WATER_QUALITY].x; st++)
	{
		...
		float2 hor = ssr_ray.r_pos.x > 0.5f ? float2(1.f, -0.1f) : float2(-0.1f, 1.f);
		...
		if (ray_check.x > 0.f)
		{
			...
			if (ray_check.x <= RayThick || (ray_depthstart + 40.f < ray_check.y))
		...
		ssr_ray.r_pos += ssr_ray.r_step * (1.f + 2.5f * PTh);
	}
