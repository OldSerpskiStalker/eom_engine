...
{
	// Get Sun dir
	float3 Sun = saturate(dot(normalize((float3)Ldynamic_dir), -normalize(P)));

	// Apply sun color
	Sun = lerp(fog_color.xyz, Ldynamic_color.rgb, Sun);
	...
	// Final fog color
	float3 FOG_COLOR = lerp(fog_color.xyz, Sun, FogBlend);
