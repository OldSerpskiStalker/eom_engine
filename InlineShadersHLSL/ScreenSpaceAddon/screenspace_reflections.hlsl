...
#include "ScreenSpaceAddon\screenspace_common.hlsl"
...
{
	float2 sky_tc = 0.f;
	float2 behind_hit = 0.f;

	// Noise to "improve" consistency between steps
	float3 noise = SSFX_noise(tc * float2(70.f,35.f) * 20.f) * q_ssr_noise[G_SSR_QUALITY];

	// Initialize Ray
	RayTrace ssr_ray = SSFX_ray_init(ray_start_vs, ray_dir_vs, 150.f, q_ssr_steps[G_SSR_QUALITY].x, 1.f);
	...
	{
		// Ray out of screen...
		if (ssr_ray.r_pos.y < 0.f || ssr_ray.r_pos.y > 1.f)
			return 0.f;

		// Trick for the horizontal out of bounds. Mirror border of the screen.
		if (ssr_ray.r_pos.x < 0.f || ssr_ray.r_pos.x > 1.f)
		{
		...
		else
		{
			...
			behind_hit *= (ray_depthstart - 2.f < ray_check.y) && NoWpnSky;
		}

		// Step the ray
		ssr_ray.r_pos += ssr_ray.r_step * (1.f + noise.x * (1.f - smoothstep(0.f, q_ssr_steps[G_SSR_QUALITY].x * 0.33f, i)));
	}
...
void SSFX_ScreenSpaceReflections(float2 tc, float4 P, float3 N, float gloss, inout float3 color, uint iSample : SV_SAMPLEINDEX)
{
	...
	float3 nw		 = mul((float3x3)m_inv_V, (float3)N);
	float3 v2point	 = mul((float3x3)m_inv_V, (float3)inVec);
	...
	float ray_fade = saturate(saturate(uvcoor.y * G_SSR_VERTICAL_SCREENFADE) + 1.f * m_terrain);

	// Adjust the intensity of MAT_FLORA
	refl_power *= m_flora ? G_SSR_FLORA_INTENSITY : 1.f;
	...
	WeaponFactor = saturate(WeaponFactor + 1.f * m_terrain);
	
	// Global intensity and limit max value.
	float main_clamp = clamp(refl_power * G_SSR_INTENSITY, 0.f, G_SSR_MAX_INTENSITY);
	...
	float wpn_clamp = clamp((refl_power + rain_extra) * G_SSR_WEAPON_INTENSITY, 0.f, G_SSR_WEAPON_MAX_INTENSITY + rain_extra);
	...
	refl_power *= saturate(1.f - (1.f - G_SSR_BEEFS_NVGs_ADJUSTMENT) * (shader_param_8.x > 0.f));
#endif
