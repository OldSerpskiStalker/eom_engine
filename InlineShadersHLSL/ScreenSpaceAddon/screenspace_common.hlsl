...
#include "Headers\lmodel.hlsl"
#include "Headers\hmodel.hlsl"

#include "ScreenSpaceAddon\screenspace_common_noise.hlsl"
#include "ScreenSpaceAddon\screenspace_common_ripples.hlsl"
...
float3 SSFX_get_scene(float2 tc, uint iSample : SV_SAMPLEINDEX)
{
	...
	float3 nw = mul( (float3x3)m_inv_V, (float3)rN );
		...
		hmodel(hdiffuse, hspecular, rMtl, rHemi, rD, (float3)rP, rN);
		...
		// hemisphere
		float3 hdiffuse, hspecular;
		hmodel(hdiffuse, hspecular, rMtl, rHemi, rD.w, (float3)rP, rN);
