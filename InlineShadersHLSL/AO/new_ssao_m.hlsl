...
#endif
{
    ...
    gbuffer_data gbd = gbuffer_load_data(GLD_P(tc, (float2)pos2d, 0));
#else
    gbuffer_data gbd = gbuffer_load_data(GLD_P(tc, (float2)pos2d, ISAMPLE));
#endif
    ...
    for (int i = 0; i < 8; i++)
    {
        float2 deltaUV = mirror(arrKernel[i], rotSample).xy * radius2D;
