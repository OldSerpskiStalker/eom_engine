...
#include "Headers\waterconfig.hlsl"
#include "Headers\watermove.hlsl"
...
vf main (v_vert v)
{
	...
	v.color	=	unpack_D3DCOLOR(v.color);
		...
		float3          N         = unpack_bx2((float3)v.N);        // just scale (assume normal in the -.5f, .5f)
		float3          T         = unpack_bx2((float3)v.T);        //
		float3          B         = unpack_bx2((float3)v.B);        //
