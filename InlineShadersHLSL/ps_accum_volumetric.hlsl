...
#endif
{
	...
	float4	t_noise	= s_noise.Sample( smp_linear, (float2)PS );
	PS.x -= time;
	PS.y -= time*0.70091f;
	//	TODO: DX10: Can use sampler with point mip filter	
	t_noise *= s_noise.Sample( smp_linear, (float2)PS );
	t_noise = t_noise*0.5f+0.5f;

	// out
	float maxIntens = I.fDensity;
	float3	result = maxIntens * s * att;
	result *= (float3)lightmap;
	result *= (float3)Ldynamic_color * (float3)t_noise;
