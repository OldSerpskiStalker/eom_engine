...
float4 main ( _input I ) : SV_Target0
{
	...
	gbuffer_data gbd0 = gbuffer_load_data( GLD_P(I.tc0.xy + 0.5f * screen_res.zw, I.pos2d.xy * 2, ISAMPLE) );
	gbuffer_data gbd1 = gbuffer_load_data( GLD_P(I.tc0.xy - 0.5f * screen_res.zw, I.pos2d.xy * 2, ISAMPLE) );
	gbuffer_data gbd2 = gbuffer_load_data( GLD_P(I.tc0.xy + 0.5f * float2(+screen_res.z, -screen_res.w), I.pos2d.xy * 2, ISAMPLE) );
	gbuffer_data gbd3 = gbuffer_load_data( GLD_P(I.tc0.xy + 0.5f * float2(-screen_res.z, +screen_res.w), I.pos2d.xy * 2, ISAMPLE) );
...
//	SSAO_OPT_DATA == 1	for full-res data
	...
	gbd = gbuffer_load_data( GLD_P(I.tc0.xy, I.pos2d.xy, ISAMPLE) );
