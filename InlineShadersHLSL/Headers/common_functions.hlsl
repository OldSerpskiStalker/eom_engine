...
{
        return        float4(low + (float3)high*high.a, 1.f);
}	
...
{
 return max(0.f,norm_w.y)*(float3)L_hemi_color;
}

float3 calc_model_lq_lighting( float3 norm_w )    
{
	return L_material.x*calc_model_hemi_r1(norm_w) + (float3)L_ambient + L_material.y*calc_sun_r1(norm_w);
}
